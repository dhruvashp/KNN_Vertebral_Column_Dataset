...
"""
This code performs KNN (Minkowski Distance Metric) as p=2 (Euclidean hence)
...
All this is only for k=5 for Minkowski p=2, Euclidean
...
for h in ite:
    knn = KNeighborsClassifier(n_neighbors=k[h],weights='distance',algorithm='auto',metric='minkowski',p=2)
    knn.fit(X,y)
    test_prediction = knn.predict(Z)
    train_prediction = knn.predict(U)
    t=0
...
"""
For Euclidean (Minkowski with p=2)

k*=6 gives minimum Test Error of 0.1
