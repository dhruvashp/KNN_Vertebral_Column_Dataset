...
y=df_train['O']
knn = KNeighborsClassifier(n_neighbors=5,weights='uniform',algorithm='auto',metric='chebyshev')
knn.fit(X,y)
...
"""
This code performs KNN (Chebyshev Distance Metric) as p=infinite
...
for h in ite:
    knn = KNeighborsClassifier(n_neighbors=k[h],weights='uniform',algorithm='auto',metric='chebyshev')
    knn.fit(X,y)
    test_prediction = knn.predict(Z)
    train_prediction = knn.predict(U)
    t=0
...
"""
For Chebyshev distance metric, an optimal K can be obtained from the curves and the error
vectors.
k=16 or k=21  give good results
...
Thus we'll take k*=16 however k* can be taken 21 too
