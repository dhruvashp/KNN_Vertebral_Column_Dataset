...
for h in ite:
    knn = KNeighborsClassifier(n_neighbors=k[h],weights='distance',algorithm='auto',metric='minkowski',p=1)
    knn.fit(X,y)
    test_prediction = knn.predict(Z)
    train_prediction = knn.predict(U)
    t=0
...
k*=26 gives minimum Test Error of 0.1

So the points needed to check around the neighboorhood is much higher
